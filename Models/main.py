...
def main():
    ...
    train_dataloader, valid_dataloader, test_dataloader = createDataLoader(
...
def train(patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_dataloader:
            print(
                f"Epoch {counter_epoch}/{num_epochs}, Batch {counter_batch}/{len(train_dataloader)}"
            )
...
def evaluation():
    # load the model,
    model_name = "RoBERTa_CODE_ONLY_COMPLETION_MODEL"
...
def collate_fn(batch):
    ...
    input_ids_padded = pad_sequence(
        [torch.nn.functional.pad(ids, (0, max_length - len(ids))) for ids in input_ids],
        batch_first=True,
        padding_value=tokeniser.pad_token_id,
    )
    attention_mask_padded = pad_sequence(
        [
            torch.nn.functional.pad(mask, (0, max_length - len(mask)))
...
def loadData():
    ...
    train_data = mw.openFile(
        "../DatasetManipulations/preprocessed/train/train_code.jsonl"
    )
    # load the validation data
    valid_data = mw.openFile(
        "../DatasetManipulations/preprocessed/valid/valid_code.jsonl"
    )
    # load the test data
    test_data = mw.openFile("../DatasetManipulations/preprocessed/test/test_code.jsonl")
...
def createDataLoader(train_data, valid_data, test_data):
    ...
    train_dataset = CodeOnlyCompletionDataset(train_data)
    valid_dataset = CodeOnlyCompletionDataset(valid_data)
    test_dataset = CodeOnlyCompletionDataset(test_data)
