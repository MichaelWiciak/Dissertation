...
def python_ast_to_ids(ast_tree, tokenizer):
    ...
    def traverse(node):
        ...
        # Map AST node to token IDs
        if isinstance(node, ast.AST):
            # Example: Map AST node to token IDs based on its type
            token_ids.extend(tokenizer.convert_tokens_to_ids([node.__class__.__name__]))
            ...
            # Convert literals to token IDs
            token_ids.extend(tokenizer.encode(str(node), add_special_tokens=False))
...
def javascript_ast_to_ids(ast_tree, tokenizer):
    # Initialize list to store token IDs
    token_ids = []
    ...
    def traverse(node):
        ...
        if isinstance(node, dict):
            # Example: Map AST node to token IDs based on its type
            token_ids.extend(tokenizer.convert_tokens_to_ids([node['type']]))
...
class ASTDocstringCompletionDataset(Dataset):
    def __init__(self, data, tokenizer, language='python'):
    ...
    def __getitem__(self, idx):
        ...
        # Assuming each example contains 'ast_tree', 'docstring_tokens', and 'output'
        ast_tree = sample['ast_tree']
        docstring_tokens = sample['docstring_tokens']
        output = sample['output']

        # Convert AST tree to token IDs using the provided tokenizer
        ast_input_ids = self.convert_ast_to_ids(ast_tree)

        # Convert docstring_tokens to token IDs
        docstring_input_ids = self.tokenizer.convert_tokens_to_ids(docstring_tokens)

        # Concatenate ast_input_ids and docstring_input_ids
        input_ids = ast_input_ids + docstring_input_ids

        # Create mask for the masked token (assuming it's in the docstring_tokens part)
        mask_index = len(ast_input_ids) + docstring_tokens.index('<mask>')
    ...
    def convert_ast_to_ids(self, ast_tree):
        # Convert AST tree to token IDs based on the language (Python or Java)
        if self.language == 'python':
            # Convert Python AST tree to token IDs
            return python_ast_to_ids(ast_tree, self.tokenizer)


        elif self.language == 'java':
            # Convert Java AST tree to token IDs
            return javascript_ast_to_ids(ast_tree, self.tokenizer)
        else:
            raise ValueError("Unsupported language. Only 'python' and 'java' are supported.")

# note: remember to do import esprima and esprima.parseScript(javascript_code), otherwise, runtime error



class ASTDocstringModel(nn.Module):
    def __init__(self, model_name):
        super(ASTDocstringModel, self).__init__()
        self.roberta = AutoModel.from_pretrained(model_name)
        self.classifier = nn.Linear(self.roberta.config.hidden_size, self.roberta.config.vocab_size)
...
def createDataLoader(train_data, valid_data, test_data):
    ...
    train_dataset = ASTDocstringCompletionDataset(train_data)
    valid_dataset = ASTDocstringCompletionDataset(valid_data)
    test_dataset = ASTDocstringCompletionDataset(test_data)
